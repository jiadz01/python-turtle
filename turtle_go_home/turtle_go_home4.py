'''turtle_go_home4.py  海龟回家4   对turtle_go_home3.py的升级优化版
本版新增了函数和命令,使游戏海龟碰车辆结束后不中断程序运行,又可按键选择重新开始.
碰撞时加一个海龟位置印章以便观察,优化并改变了程序结构,更规范.
主要技术要点:
...
其又进一步改写和扩展版本详见:turtle_go_home5.py,turtle_go_home.py
'''
...
def writer_up():
    ...
    writer.goto(-236, 272) #书写海龟笔拟写游戏第几关文字说明的中心坐标位置
    writer.write(f"level:{level}", align="center", font=FONT)
    level += 1

#自定义文字提示游戏失败,结束函数
def writer_failed():
    writer.goto(-166, 250) #书写海龟笔写下边语句文字说明的中心坐标位置
    FONT = ("Courier", 18, "bold") #设置字体名字、字大小、字类型
    writer.pencolor("red") # 设置海龟画笔颜色
    writer.write("Crash! Game Over.",align="center", font=FONT)
...
def writer_reset(x=0,y=-210):
    ...
    writer.write("Press space key to start play.",
          align="center", font=("Courier", 22, "bold")) #显示出操作提示文字pass
    writer.goto(x,y-40)
...
def main():
    ...
    number = car_number #保留车辆数设置供重新(失败后)启动游戏主循环前期处置
    writer_reset(0,0) ####
