'''clock_3.py 时钟 是clock_2.py的修改版本
海龟有三种模式:standard模式海龟初始朝右,正数角度是逆时方向转(默认)
              logo模式海龟初始朝上,正数角度是顺时方向转
              world模式是使用户自定义世界坐标系
本例将使用系统默认的standard模式,对比clock_2.py编码可进一步理解这两个海龟模式的区别:
   初始化时将海龟朝向左转90度,方向朝上.指针转动时以负度数,即可顺时针转动
   下边修改或增加的语句注释尾加有* ,可去与原程序比较
本例取消了异常处置结构try-except.
...
def make_hand_shape(name, laenge, spitze, width=0): #指针长laenge,指针头三角形边长spitze,指针宽width
    ...
    turtle.lt(90) #画笔左转90度,由原来朝右变成朝上 *
...
def setup():
    ...
    turtle.mode("standard") #设置海龟模式,有三种模式,standard海龟初始朝右,正数角度是逆时方向转 *
    ...
    writer.lt(90) #画笔左转90度,由原来朝右变成朝上 *
...
def main():
    ...
    turtle.onkeypress(turtle.bye,"Q") #按键盘Q键调用Screen()的专有方法bye(),退出程序运行
    return "clock_3.py:   main() END  EVENTLOOP"
