'''clock_1.py 时钟 是从turtle文档中的演示程序集中(turtledemo)下载,
   原程序tdemo_clock.py是在一个专用的演示查询器上运行,
   原程下载后,修改导入模块方式并对每条语句进行中文注释,以便学习.
   原程序不能退出,只能直接关闭绘画窗口退出.
...
#定义注册指针形状函数,将一个海龟形状加入TurtleScreen的形状列表,3个形参,name为字符串变量,多边形的调用名称
def make_hand_shape(name, laenge, spitze): #指针长laenge,指针头三角形边长spitze
    ...
    hand(laenge, spitze) #调自定义画时钟指针函数hand()画指针
...
def setup():
    ...
    make_hand_shape("second_hand", 125, 25) #调定义注册指针形状函数,加入秒针形状,调用名称second_hand
    make_hand_shape("minute_hand",  130, 25) #调定义注册指针形状函数,加入分针形状,调用名称minute_hand
    make_hand_shape("hour_hand", 90, 25) #调定义注册指针形状函数,加入时针形状,调用名称hour_hand
    ...
    for hand in second_hand, minute_hand, hour_hand: #利用for循环对这三个海龟对象进行如下设置
        ...
        hand.shapesize(1, 1, 3) #基于拉伸因子调整海龟外观,垂直于朝向拉伸因子=1,平行于朝向拉伸因子=1
