''' color_rgb.py 其功能是显示RGB颜色模型的调色变化,窗口背景颜色就是这三原色各值的叠加结果.
1.由color_2.py改写并对每条语句注释,变成turtle绘图调色实用工具(获取各种颜色的RGB值).
2.在绘画窗口顶部的标题栏中显示turtle三种RGB颜色模式的值,括号外是RGB 1.0模式的值,括号内是值RGB 255模式的值,
  前边带#的是16进制编码值.

使用方法:1.将鼠标光标分别移到调色海龟上边，按住鼠标左键上下拖动海龟调颜色值大小,可见窗口背景颜色动态变化.
        2.按键盘p键,弹出文本字符串输入窗口,用户可对所选颜色输入名称或编号,该所选颜色的RGB值会在Python
          shell窗口或界面打印出(临时记录).
        3.按键盘Q键退出程序运行,并关闭绘画窗口.
注意:
...
'''
import turtle #导入turtle模块，调用其对象、类和函数等前加turtle.(指明调用对象的从属关系)

#定义调色海龟笔 类 ColorTurtle()
class ColorTurtle(turtle.Turtle):  #创建调色海龟子类ColorTurtle,它继承的父类是Turtle,类名的首字母大写,
                            #如无继承而新建类则括号是空的
    def __init__(self, x, y):   #类的专有方法__init__(),调用时自动传入调用实参self,只需对形参x,y给出实参
                                #这里x,y是画笔海龟的初始坐标.
        turtle.Turtle.__init__(self) #让父类Turtle的属性与子类ColorTurtle关联才能使用,如无继承而新建类则无此命令行.
...
def records(): #在Python shell窗口或界面打印出所选颜色的RGB值(临时记录)
    ...
    #调用textinput()进行弹出窗口和文本字符串输入
    a = turtle.textinput('Temporary record','Enter a name for the selected colorq')
    if a == None or a == '':
        print('RGB Temporary records ------------')
    else:
        print('RGB Temporary records ---'+a )
    print('255 Mode: '+rgb_255)
    print('1.0 Mode: '+rgb_1)
    print('Hexadecimal code: '+rgb_16)
    #调用textinput()后将关闭所有键盘事件驱动,调完成后应用以下命令恢复
...
def main():
    ...
    screen = turtle.Screen()   #创建一个我们命名为screen的Screen实例，用来显示海龟调色的屏幕窗口
